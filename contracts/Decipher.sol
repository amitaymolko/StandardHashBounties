...
contract Decipher {
    ...
    event FailedDecipherEvent(address predictor, uint blockNumber, uint predictionIndex);
    event DecipherEvent(address predictor, uint blockNumber, uint predictionIndex);
    ...
    function claimReward(uint _index) public {
        ...
        if (!prediction.won && block.blockhash(prediction.predictedBlock) == prediction.predictedHash) {
            ...
            DecipherEvent(prediction.predictor, block.number, _index);
        } else {
            FailedDecipherEvent(prediction.predictor, block.number, _index);
